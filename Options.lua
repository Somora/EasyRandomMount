...
function ERM:RefreshOptions()
    ...
    if self.generalPanel then
        ...
        self.generalPanel.breathSurfaceCheck:SetChecked(db.preferFlyingWhenBreathTimerMissing)
...
mainPanel.breathSurfaceCheck = CreateFrame("CheckButton", nil, mainPanel, "InterfaceOptionsCheckButtonTemplate")
mainPanel.breathSurfaceCheck:SetPoint("TOPLEFT", mainPanel.surfaceCheck, "BOTTOMLEFT", 0, -8)
mainPanel.breathSurfaceCheck.Text:SetText("Prefer flying when breath timer is hidden")
mainPanel.breathSurfaceCheck:SetScript("OnClick", function(self)
    ERM:GetDB().preferFlyingWhenBreathTimerMissing = self:GetChecked()
    ERM:ClearMountCache()
end)

mainPanel.flyableCheck = CreateFrame("CheckButton", nil, mainPanel, "InterfaceOptionsCheckButtonTemplate")
mainPanel.flyableCheck:SetPoint("TOPLEFT", mainPanel.breathSurfaceCheck, "BOTTOMLEFT", 0, -8)
